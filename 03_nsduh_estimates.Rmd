...
## CAUTION: The `stan_fit` objects are NOT variance-adjusted to correct for
##          dependencies between observations in the NSDUH sampling design.
##          The `stan_fit` objects are survey-weighted posterior distributions 
##          that correct results for unequal probabilities of selection and
...
# Rename parameter labels and save the model fit object as an array
# This is done to aid chart comprehension
pyMDE_m1_array <- renameArray(model = pyMDE_m1,
                              names = groupName,
                              start = 48,
                              finish = 89)
# 10/24/2024 Note: this code was updated to capture the correct intercept,
# which is "b_Intercept".
# In a prior version of this code, this was incorrectly identified as the 
# parameter "Intercept" -- the incorrect "Intercept" parameter is mean-centered
# and is much lower than the correct "b_Intercept" parameter, which is on the
# original scale.
# Therefore, this produced lower prevalence estimates for each intersectional
# group in our original analysis and publication.
dimnames(pyMDE_m1_array)[[3]][c(1:3, 5, 91)] <-
  c("age_2534", "age_3549", "age_50plus", 
    "Between-group std. dev.", "b_Intercept")

# define parameters to display in plots
parameters <- c(c(dimnames(pyMDE_m1_array)[[3]][c(1:3, 5, 91)], 
                  groupName[1:42]))
...
pyMDE_prevalence <-
  age_adj_prevalence(model = pyMDE_m1, 
                     # 10/24/2024 Note: We have updated this to the correct
                     # intercept parameter ("b_Intercept")
...
pyMDE_m1_estimates <- pyMDE_m1_sum %>% 
  ...
  # ("b_Intercept")
  filter(rownames(.) %in% c("b[1]", "b[2]", "b[3]", "b_Intercept")) %>% 
  mutate(
    # Create a new column named "parameter" to store parameter names
    parameter = c("26-34",
                  "35-49",
                  "50+",
                  "Intercept"),
...
dimnames(pyMDE_m2_array)[[3]][c(1:12, 14, 100)] <-
  c("age_2534", "age_3549", "age_50plus",
    "Asian", "Black", "Hispanic/Latine", 
    "NAAN", "NHPI", "Multiracial",
    "Woman", "Gay/Lesbian", "Bisexual",
    "Between-group std. dev.", "b_Intercept")


# define parameters to display in plots
parameters <- dimnames(pyMDE_m2_array)[[3]][c(1:12, 14, 100)]
...
pyMDE_effects <- 
  calculate_effects(model = pyMDE_m2,
                    # 10/24/2024 Note: Updated to have the correct intercept
...
# Rename parameter labels save the model fit object as an array
# This is done to aid chart comprehension
...
# Therefore, this produced lower prevalence estimates for each intersectional
# group in our original analysis and publication.
dimnames(ltMDE_m1_array)[[3]][c(1:3, 5, 91)] <-
  c("age_2534", "age_3549", "age_50plus", 
    "Between-group std. dev.", "b_Intercept")

# define parameters to display in plots
parameters <- c(dimnames(ltMDE_m1_array)[[3]][c(1:3, 5, 91)],
                groupName[1:42])
...
# save the model fit object as an array
...
# Therefore, this produced lower prevalence estimates for each intersectional
# group in our original analysis and publication.
dimnames(ltMDE_m2_array)[[3]][c(1:12, 14, 100)] <-
  c("age_2534", "age_3549", "age_50plus",
    "Asian", "Black", "Hispanic/Latine", 
    "NAAN", "NHPI", "Multiracial",
    "Woman", "Gay/Lesbian", "Bisexual",
    "Between-group std. dev.", "b_Intercept")
# define parameters to display in plots
parameters <- c(dimnames(ltMDE_m2_array)[[3]][c(1:12, 14, 100)])
